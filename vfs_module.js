...
sack.objectStorage.prototype.put = function( obj,sign ) {
	...
	var container = new objectStorageContainer(obj,sign);
	...
	var storage = this.stringifier.stringify( container );
...
sack.objectStorage = function (...args) {
	...
	function objectToJSOX(){
		...
		if( exist ) {
			...
			newStorage.stored.set( this, this.id );
		}
		return this;
	}
	...
	newStorage.parser = sack.JSOX.begin( parserObject );

	newStorage.parser.registerPromiseFromJSOX( "~os", (objId,res,rej)=>{
		if( newStorage.mapping ) {
			var exist = newStorage.cached.get( objId );
			if( !exist ) {
				newStorage.get( objId ).then( (obj)=>{
					newStorage.cached.set( objId, obj );
					newStorage.stored.set( obj, objId );
					res( obj );
				} );
			}
		} else {
