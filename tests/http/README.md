...
**Four files are deliberately plain node** — `wirepeer.mjs`, `rawpipe.mjs`,
`splitpeer.mjs` and `exittest.mjs` — and must be run *without*
`--import sack.vfs/import` (`splitfuzz.mjs` spawns its peer itself):
...
the only one that exercises the parser's resume path.
