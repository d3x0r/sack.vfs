	...
	if( args.IsConstructCall() ) {
...

		{
			...
			if( opts->Has( optName = strings->argString->Get( isolate ) ) ) {
				...
				if( opts->Get( optName )->IsString() ) {
					...
					for( n = 0; n < arr->Length(); n++ ) {
						argArray[n] = StrDup( *String::Utf8Value( arr->Get( n )->ToString() ) );
					}
					argArray[n] = NULL;
				}
			}
			if( opts->Has( optName = strings->workString->Get( isolate ) ) ) {
				Local<Value> val;
				if( opts->Get( optName )->IsString() )
					work = new String::Utf8Value( opts->Get( optName )->ToString() );
			}
			if( opts->Has( optName = strings->binString->Get( isolate ) ) ) {
				Local<Value> val;
				if( opts->Get( optName )->IsString() )
					bin = new String::Utf8Value( opts->Get( optName )->ToString() );
			}
